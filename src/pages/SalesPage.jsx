...
import { Plus, Search, Eye, Ban, Receipt, Edit2 } from 'lucide-react'
...
export default function SalesPage() {
  ...
  }, [page, searchInput, filters.fecha_desde, filters.fecha_hasta])
  ...
  const openCreate = () => { setEditingId(null); setForm({ items: [] }); setModalOpen(true) }
                ...
                <tbody className="divide-y divide-slate-200 dark:divide-slate-700">
                  {form.items.map((item, i) => (
                    <tr key={i}>
                      ...
                      <td className="px-3 py-2">
                        <button type="button" onClick={() => removeItem(i)} className="text-red-500 hover:text-red-700 p-1"><Ban className="h-4 w-4" /></button>
                      </td>
                    </tr>
                  ))}
                </tbody>
        ...
        {detailData ? (
          <div className="space-y-4">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
              ...
              <div><span className="text-slate-500">Método:</span> <span className="text-slate-900 dark:text-slate-100">{detailData.payment_method || 'cash'}</span></div>
            </div>
            <div className="overflow-x-auto border rounded-lg border-slate-200 dark:border-slate-700">
              <table className="w-full text-sm">
